...
public:
	...
	Setka(string name);                   // Считывание сетки с файла
	...
	void Construct_fast(void);
	// Построение ячеек по векторам кандидатам в соседей
	...
	bool Reconstruct_medium2(bool wide = true);
	bool Reconstruct_medium3(bool wide = true);
	bool Reconstruct_medium4(bool wide = true);   // То же что и функция 3, но для абсолютно всех ячеек сетки
	void renumber(void);
	...
	void save_soseds(); // Сохранить текущих соседей у ячеек в вектор стандартных соседей.
	...
	void Add_par(void);                     // Добавление пар к уже имеющимся парам
	...
	void Calc_normal2(void);

	void Smooth(void); // сгладить поверзность пар.
	...
	void Save_setka(string name);            // Сохранение сетки (геометрия + параметры)
